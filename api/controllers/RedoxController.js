...
//////////////////////////////////////////////////////////
var VERIFICATION_TOKEN = "mytoken";
var REDOX_API_KEY = "d56c68cf-cc30-470f-a94e-52afdab046f0";
var REDOX_API_SECRET =
  "bNIFwwXUcouAvokT2s1ivPLpdPYKeLyYs4wcG3zfWaod7F005DIqwYWSwC6fTCrc8imrwwzY";
//////////////////////////////////////////////////////////

var REDOX_AUTH_URL =
  process.env.REDOX_AUTH_URL || "https://api.redoxengine.com/auth/authenticate";
var REDOX_API_URL =
  process.env.REDOX_API_URL || "https://api.redoxengine.com/endpoint";
...
module.exports = {
    ...
    Transmission.create(transmisison, function (err, t) {
      if (err) {
        ...
        return res.badRequest(
          "Error occurred while saving transmission in application."
        );
      }
      ...
      request.post(options, function (err, response, body) {
        ...
        Message.create(message, function (err, m) {
          if (err) {
            sails.log(err);
            return res.badRequest(
              "Error occurred while saving the message in application."
            );
          }
  ...
  patientSearch: function (req, res) {
    // Ensure we have a valid requestToken to use
    requestToken(req.body, function (err, accessToken) {
      if (err) {
        return res.badRequest(err);
      ...
      // Validate the contents of the message request
      if (!req.body || !req.body.data) {
        return res.badRequest("No data provided");
      }
      ...
      request.post(options, function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
          return res.send(JSONParser.prettyPrint(response.body));
        }
        // Save the returned details to our database
        Patient.create({ body: response.body }, function (err, m) {
          if (err) {
            sails.log(err);
            return res.badRequest("Error occurred while saving the details");
          }
